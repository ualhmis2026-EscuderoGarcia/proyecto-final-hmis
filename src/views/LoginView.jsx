...
const LoginView = () => {
  ...
  const handleLogin = async (e) => {
    ...
    // Si tiene éxito, App.jsx (que escucha a onAuthStateChange) se encargará de actualizar la vista.
  };
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-slate-900">
      ...
      <div className="relative z-10 w-full max-w-md p-8 bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl">
        <div className="mb-10 text-center">
          <h2 className="text-3xl font-black text-white tracking-tight mb-2">Bienvenido de vuelta</h2>
          <p className="text-sm text-indigo-200/80 font-medium">Ingresa a tu espacio de trabajo seguro</p>
        </div>

        {error && (
          <div className="mb-6 p-3 rounded-xl bg-red-500/20 border border-red-500/50 text-red-200 text-sm font-medium text-center backdrop-blur-md">
            {error}
          </div>
        )}
          ...
          >
            ...
            <div className="relative flex items-center justify-center w-full bg-gradient-to-r from-blue-500 to-indigo-500 py-3.5 px-4 rounded-xl shadow-inner transition-all group-active:scale-[0.98]">
              {isLoading ? (
                ...
                <span className="text-sm font-bold text-white tracking-wide">Iniciar Sesión</span>
              )}
            </div>
          </button>
        </form>
